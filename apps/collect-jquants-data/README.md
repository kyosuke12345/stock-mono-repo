...
| 変数 | 必須 | 説明 |
| --- | --- | --- |
| `JQUANTS_API_KEY` | ✅ | J-Quants ダッシュボードで発行した API キー（`x-api-key`） |
...
1. `/equities/master` で取得したデータを `company_info` テーブルに upsert
2. `/equities/bars/daily` で取得したデータを `daily_stock_info` テーブルに upsert
3. `/fins/summary` で取得したデータを `fins_statement` テーブルに upsert
